 ...
 *      binary operators. 
 * 
 *  [lyn, 2015 Jun 12]: finally wrap up! 
 ...
 *   + Collapsed blocks
 *   + Disabled blocks
 *   + Testing of my projects
 *   + Testing of power users projects
 *   ==> SOMETHING IN XML TO SAY IT WAS CONVERTED!!!
 *   + Test various procedure error cases
 *   + Problematic blocks:
 *     - none color block 
 *     - Screen openScreenAnimation and closeScreenAnimation methods
 *     - trailing spaces in text box not property handled? E.g. ScreenOps app, " closed with value" vs. " closed with value "
...
function convert_AI1_XML_to_AI2_XML(filename, AI1_XML) { // Both AI1_XML and AI2_XML are strings
  if (AI1_XML.length == 0) {
    reportError("AI1 file " + filename + " is empty!");
  ...
  try {
    ...
    reportError("Caught error in parseBlocks: " + err.message); 
    return {xml: goog.dom.createDom('xml'), numBlocks: 0}; 
  }
    // var keys = [];
  ...
  try {
    ...
    reportError("Caught error in convertBlocks: " + err.message); 
    return {xml: goog.dom.createDom('xml'), numBlocks: 0}; 
  }
}
  ...
  */ 
  ...
  if (orphanedBlockIds.length > 0) {
    reportWarning("The result of conversion has " + orphanedBlockIds.length + " orphaned block assemblies."
                  // + "(for input block ids [" + orphanedBlockIds + "])."
...
function convertBlock(id, maps) {
  ...
  try {
    ...
    reportError("Caught error in convertBlock of id " + id + ": " + err.message); 
    resultBlock = undefined;
  }
}
...
/*----------------------------------------------------------------------
 Convert component properties, events, and methos
 ----------------------------------------------------------------------*/
...
var AI1ConversionMap = 
{
  ...
  // Note: component events and methods are added by addComponentEntriesToAIConversionMap() below
  ...
  // generic methods are added by addComponentEntriesToAIConversionMap(), e.g., "Type-Ball-PointInDirection"
  ...
  */
  ...
  "string-replace-all": {convert: convertOperator, type:"text_segment", argNames: ["TEXT", "SEGMENT", "REPLACEMENT"], kind: "expression"}, 
...
function addComponentEntriesToAI1ConversionMap() {
  ...
  // Special cases that overwrite above:
  AI1ConversionMap["Screen-OpenScreenAnimation"] 
    = {convert: convertUnimplemented, 
       type: "unimplemented_screen_openScreenAnimation", 
       message: "Conversion of the Screen OpenScreenAnimation method has not been implemented."
                 + " It needs to be converted to a Screen property setter.",
       kind: "statement"};
  AI1ConversionMap["Screen-CloseScreenAnimation"]
    = {convert: convertUnimplemented, 
       type: "unimplemented_screen_closeScreenAnimation", 
       message: "Conversion of the Screen CloseScreenAnimation method has not been implemented."
                + " It needs to be converted to a Screen property setter.",
       kind: "statement"};
  AI1ConversionMap["Type-OpenScreenAnimation"]
    = {convert: convertUnimplemented, 
       type: "unimplemented_screen_component_openScreenAnimation", 
       message: "Conversion of the Screen OpenScreenAnimation generic component method has not been implemented."
                 + " It needs to be converted to a generic Screen property setter.",
       kind: "statement"};
  AI1ConversionMap["Screen-CloseScreenAnimation"]
    = {convert: convertUnimplemented, 
       type: "unimplemented_screen_component_closeScreenAnimation", 
       message: "Conversion of the Screen CloseScreenAnimation generic component method has not been implemented."                 
                + " It needs to be converted to a generic Screen property setter.",
       kind: "statement"};
