...
Blockly.Versioning.isDom = function (blocksRep) {
  try {
    ...
    // In testing context, HTMLUnknownElement is undefined and causes an error,
...
Blockly.Versioning.ensureDom = function (blocksRep) {
  if (Blockly.Versioning.isDom(blocksRep)) {
    ...
    console.log("Number of top blocks is " + Blockly.mainWorkspace.getTopBlocks(true).length); 
    var dom = Blockly.Xml.workspaceToDom(blocksRep);
    console.log("Number of dom children is " + Blockly.Versioning.getBlockChildren(dom).length);
    return dom;
  } else {
...
Blockly.Versioning.getBlockChildren = function (dom) {
    ...
    for (var gdi = 0, gdChild; gdChild = gdChildren[gdi]; gdi++) {
      // console.log("gdChild " + gdi + ": " + Blockly.Xml.domToText(gdChild));
      result.push(gdChild);
    }
    return result;
}
...
Blockly.Versioning.ensureWorkspace = function (blocksRep) {
  if (Blockly.Versioning.isWorkspace(blocksRep)) {
    ...
    console.log("Number of dom children is " + Blockly.Versioning.getBlockChildren(blocksRep).length);
    Blockly.mainWorkspace.clear(); // Remove any existing blocks before we add new ones.
    // ***lyn***
    console.log("Blockly.mainWorkspace.clear()");
    // console.log("mainWorkspace.topBlocks_.length = " + Blockly.mainWorkspace.topBlocks_.length); 
    console.log("Number of top blocks is " + Blockly.mainWorkspace.getTopBlocks(true).length);  
    /*console.log("XML blocks rep is " + Blockly.Xml.domToText(blocksRep));
    var gdChildren = goog.dom.getChildren(blocksRep);
    for (var gdi = 0, gdChild; gdChild = gdChildren[gdi]; gdi++) {
      console.log("gdChild " + gdi + ": " + Blockly.Xml.domToText(gdChild)); 
    }
    /*var cChildren = blocksRep.children;
    for (var ci = 0, cChild; cChild = cChildren[ci]; ci++) {
      console.log("cChild " + ci + ": " + Blockly.Xml.domToText(cChild)); 
    }
    * /
    var cnChildren = blocksRep.childNodes;
    for (var cni = 0, cnChild; cnChild = cnChildren[cni]; cni++) {
      console.log("cnChild " + cni + ": " + Blockly.Xml.domToText(cnChild)); 
    */
    Blockly.Xml.domToWorkspace(Blockly.mainWorkspace, blocksRep);
    console.log("Blockly.Xml.domToWorkspace(Blockly.mainWorkspace, blocksRep)");
    console.log("Number of top blocks is " + Blockly.mainWorkspace.getTopBlocks(true).length); 
    // ***lyn***
    // console.log("mainWorkspace.topBlocks_.length = " + Blockly.mainWorkspace.topBlocks_.length);
    // console.log("Number of top blocks is " + Blockly.mainWorkspace.getTopBlocks(true).length);
...
Blockly.Versioning.xmlBlockTextToDom = function(xmlBlockText) {
  ...
  topLevelXmlString = "<xml>" + xmlBlockText + "</xml>"
