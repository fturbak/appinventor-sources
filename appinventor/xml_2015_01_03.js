...
Blockly.Xml.workspaceToDom = function(workspace) {
  console.log("Blockly.Xml.workspaceToDom()");
  var width;  // Not used in LTR.
  if (Blockly.RTL) {
    width = workspace.getMetrics().viewWidth;
  }
  var xml = goog.dom.createDom('xml');
  console.log("Blockly.Xml.workspaceToDom: # xml children=" + Blockly.Versioning.getBlockChildren(xml).length);
  var blocks = workspace.getTopBlocks(true);
  console.log("Blockly.Xml.workspaceToDom: number of top blocks=" + blocks.length);
  for (var i = 0, block; block = blocks[i]; i++) {
    var element = Blockly.Xml.blockToDom_(block);
    console.log("Blockly.Xml.workspaceToDom: element #=" + i + "=" + Blockly.Xml.domToText(element));
      ...
      function () {
        ...
        for (var x = 0, xmlChild; xmlChild = xml.childNodes[x]; x++) {
          if (xmlChild.nodeName.toLowerCase() == 'block') {
            ...
            // ***lyn***
            console.log("Blockly.Xml.domToBlock: block=" + Blockly.Xml.domToText(Blockly.Xml.blockToDom_ (block)));
